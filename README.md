# Аннотация
Здесь представлены работы, решение технических заданий по дисциплине "Технологии программирования" направления "Бизнес-информатика" студента НИУ ВШЭ Малеванной Ренаты Борисовны группы ББИ236.
...
- [Техническое задание №2 (ТЗ2)](https://github.com/RenataMal/Tech-Programming/tree/main?tab=readme-ov-file#%D1%82%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5-%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5-2-%D1%82%D0%B72)
- [Техническое задание №3 (ТЗ3)](https://github.com/RenataMal/Tech-Programming/tree/main?tab=readme-ov-file#%D1%82%D0%B5%D1%85%D0%BD%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5-%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5-3-%D1%82%D0%B73)
...
- `main`: Это основная функция, которая запускает программу. В этой функции происходит чтение чисел из файла, вызов функций для нахождения минимального, максимального, суммарного и произведения чисел, а также вывод результатов, также вывод тестов производительности, описание которых чуть ниже.
...
Эти функции предоставляют основную функциональность программы, позволяя читать числа из файла и выполнять различные вычисления над ними.
...
Представлен график зависимости времени выполнения нахождения минимума от кол-ва чисел в файле. Время выполнения поиска минимума возрастает с увеличением количества чисел в файле. Это наблюдение подтверждает, что алгоритму требуется больше времени для обработки больших объемов данных. (от 10 до 1 000 000 элементов), (числа генерируются в диапозоне 100). Асимптотика O(n).
...
Мобильное приложение реализует сервис заказа доставки еды. В основе его функционирования лежит взаимодействие трёх ключевых акторов: пользователей, ресторанов и курьеров.
Вначале пользователь регистрирует аккаунт в системе, после чего осуществляет вход для доступа к функционалу. Затем, выбрав один из ресторанов, пользователь ознакамливается с представленным в нем меню и формирует свой заказ, выбирая интересующие его блюда.
После оформления заказа, пользователь оплачивает его. В случае недостаточных средств на счету происходит повторная оплата.
После успешной оплаты система подтверждает заказ и передает его в ресторан, где начинается процесс приготовления. После приготовления заказ передается курьеру, который принимает его на доставку.
В процессе доставки курьер транслирует свое местположение. Пользователь в свою очередь может отслеживать статус заказа в режиме реального времени, узнавая его местоположение.
По завершении доставки пользователь получает заказ и завершает процесс. 

Здесь представлены диаграммы [вариантов использования](https://github.com/RenataMal/Tech-Programming/blob/main/TZ3/%D0%94%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0_%D0%B2%D0%B0%D1%80%D0%B8%D0%B0%D0%BD%D1%82%D0%BE%D0%B2_%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F.drawio.png), [последовательности для процесса заказа еды через приложение](https://github.com/RenataMal/Tech-Programming/blob/main/TZ3/%D0%94%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0_%D0%BF%D0%BE%D1%81%D0%BB%D0%B5%D0%B4%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B5%D0%B9.drawio.png), [деятельности для описания процесса обработки заказа системой](https://github.com/RenataMal/Tech-Programming/blob/main/TZ3/%D0%94%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0_%D0%B4%D0%B5%D1%8F%D1%82%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8.drawio.png), [диаграмма состояний заказа](https://github.com/RenataMal/Tech-Programming/blob/main/TZ3/%D0%94%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0_%D1%81%D0%BE%D1%81%D1%82%D0%BE%D1%8F%D0%BD%D0%B8%D0%B9.drawio.png), оформленные в draw.io.
